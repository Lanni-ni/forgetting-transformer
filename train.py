import itertools  # 添加这一行用于循环 dataloader
...
# 🆕 动态 ALiBi: 更新 epoch 的辅助函数
def update_dynamic_fgate_epoch(model, token_count, tokens_per_epoch, config):
    try:
        if hasattr(model, 'update_fgate_epoch'):
            use_dynamic = getattr(config.model.config, 'use_dynamic_fgate', False)
            if use_dynamic:
                current_epoch = int(token_count / tokens_per_epoch)
                max_epochs = getattr(config.model.config, 'fgate_num_epochs', 10)
                current_epoch = min(current_epoch, max_epochs)
                model.update_fgate_epoch(current_epoch)
                return current_epoch
    except Exception as e:
        logging.warning(f"Failed to update dynamic fgate epoch: {e}")
    return None

def update_dynamic_alibi_epoch(model, token_count, tokens_per_epoch, config):
    """根据 token 数量更新动态 ALiBi 的 epoch"""
    try:
        if hasattr(model, 'update_alibi_epoch'):
            use_dynamic = getattr(config.model.config, 'use_dynamic_alibi', False)
            if use_dynamic:
                current_epoch = int(token_count / tokens_per_epoch)
                max_epochs = getattr(config.model.config, 'alibi_num_epochs', 10)
                current_epoch = min(current_epoch, max_epochs)
                model.update_alibi_epoch(current_epoch)
                return current_epoch
    except Exception as e:
        logging.warning(f"Failed to update dynamic ALiBi epoch: {e}")
    return None
...
def train(config: Config):
    ...
    if hasattr(model, 'setup_dynamic_fgate'):
        model.setup_dynamic_fgate(config.model.config)
    ...
    # 🆕 动态 ALiBi: 计算每个 epoch 的 token 数量
    tokens_per_epoch = config.train.max_tokens // getattr(config.model.config, 'alibi_num_epochs', 10)
    last_logged_epoch = -1
    
    # 关键修改：使用 itertools.cycle 让 dataloader 循环多个 epochs
    for train_data in itertools.cycle(train_dataloader):
        ...
        # 🆕 动态 ALiBi: 更新 epoch
        current_epoch = update_dynamic_alibi_epoch(
            model, 
            train_state["token_count"], 
            tokens_per_epoch, 
            config
        )
        
        # 🆕 Dynamic Forgetting Gate: update epoch
        fgate_epoch = update_dynamic_fgate_epoch(
            model, train_state["token_count"], tokens_per_epoch, config
        )
        if fgate_epoch is not None and fgate_epoch != last_logged_epoch:
            if hasattr(model, 'get_current_lambda'):
                current_lambda = model.get_current_lambda()
                # Get bias stats to monitor offset
                bias_stats = {}
                if hasattr(model, 'get_bias_stats'):
                    bias_stats = model.get_bias_stats()
                bias_str = " ".join([f"L{k.split('layer')[1]}={v:.4f}" for k, v in bias_stats.items() if 'mean' in k])
                logging.info(f"Dynamic Fgate - Epoch {fgate_epoch}: lambda={current_lambda:.4f} | bias_mean: {bias_str}")
                metrics = {"dynamic_fgate/epoch": fgate_epoch, "dynamic_fgate/lambda": current_lambda}
                metrics.update(bias_stats)
                logger.log_metrics(
                    metrics=metrics,
                    step=train_state["token_count"],
                    logger_names=("wandb", "jsonl"),
                    logger_kwargs={"jsonl": {"filename": "dynamic_fgate"}},
                )

        # 🆕 动态 ALiBi: 记录工作记忆容量
        if current_epoch is not None and current_epoch != last_logged_epoch:
            if hasattr(model, 'get_working_memory_capacity'):
                w_t = model.get_working_memory_capacity()
                m_t = 1.0 - w_t
                logging.info(f"🧠 Dynamic ALiBi - Epoch {current_epoch}: m_t={m_t:.4f}, w_t={w_t:.4f}")
                logger.log_metrics(
                    metrics={
                        "dynamic_alibi/epoch": current_epoch,
                        "dynamic_alibi/m_t": m_t,
                        "dynamic_alibi/w_t": w_t,
                    },
                    step=train_state["token_count"],
                    logger_names=("wandb", "jsonl"),
                    logger_kwargs={"jsonl": {"filename": "dynamic_alibi"}},
                )
        ...
        if train_state["token_count"] % config.log_interval == 0:
            ...
            if throughput_monitor.can_compute():
                pbar_metrics.update({
                    ...
                    "MFU": throughput_metrics.get("throughput/mfu_total_recent", 0),
                    "TFLOPS": throughput_metrics.get(
                        "throughput/flop_count_per_second_total_recent", 0
                    ) / 1e12,
                })
            pbar.display(
                step=train_state["token_count"],
