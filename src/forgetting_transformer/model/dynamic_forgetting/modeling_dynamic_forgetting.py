...
try:
    ...
    try:
        ...
        from forgetting_transformer.model.dynamic_forgetting.configuration_dynamic_forgetting import DynamicForgettingConfig
...
class ForgettingAttentionLayer(nn.Module):
    ...
    def __init__(
        ...
        # Dynamic forgetting gate state
        self.use_dynamic_fgate = False
        self.fgate_initial_lambda = 1.0
        self.fgate_decay_rate = 0.6
        self.current_epoch = 0
        self._last_log_fgate_mean = None
        self._last_log_fgate_std = None
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Record raw log_fgate stats BEFORE scaling (to detect offset)
        if self.use_dynamic_fgate:
            with torch.no_grad():
                self._last_log_fgate_mean = log_fgate.mean().item()
                self._last_log_fgate_std = log_fgate.std().item()
            dynamic_lambda = self.fgate_initial_lambda * (self.fgate_decay_rate ** self.current_epoch)
            log_fgate = log_fgate * dynamic_lambda
...
class DynamicForgettingMLP(nn.Module):
...
class DynamicForgettingBlock(nn.Module):
...
class DynamicForgettingPreTrainedModel(PreTrainedModel):

    config_class = DynamicForgettingConfig
    supports_gradient_checkpointing = True
    _no_split_modules = ['DynamicForgettingBlock']

    def __init__(self, config, *inputs, **kwargs):
        # 动态修复 config_class 以支持远程代码加载
        if hasattr(config, '__class__'):
            config_module = config.__class__.__module__
            if 'transformers_modules' in config_module or config_module == 'configuration_dynamic_forgetting':
...
class DynamicForgettingModel(DynamicForgettingPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([DynamicForgettingBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        # if output_attentions:
            # warnings.warn(
                # "`DynamicForgettingModel` does not support output attention weights now, so `output_attentions` is set to `False`."
            # )
...
class DynamicForgettingForCausalLM(DynamicForgettingPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = DynamicForgettingModel(config)
        self.vocab_size = config.vocab_size
    ...
    def update_fgate_epoch(self, current_epoch: int):
        for layer in self.model.layers:
            if hasattr(layer.attn, 'current_epoch'):
                layer.attn.current_epoch = current_epoch
    
    def setup_dynamic_fgate(self, config):
        use_dynamic = getattr(config, 'use_dynamic_fgate', False)
        if use_dynamic:
            for layer in self.model.layers:
                layer.attn.use_dynamic_fgate = True
                layer.attn.fgate_initial_lambda = getattr(config, 'fgate_initial_lambda', 1.0)
                layer.attn.fgate_decay_rate = getattr(config, 'fgate_decay_rate', 0.6)
    
    def get_current_lambda(self):
        layer = self.model.layers[0]
        epoch = layer.attn.current_epoch
        return layer.attn.fgate_initial_lambda * (layer.attn.fgate_decay_rate ** epoch)
    
    def get_bias_stats(self):
        stats = {}
        for i, layer in enumerate(self.model.layers):
            attn = layer.attn
            if attn._last_log_fgate_mean is not None:
                stats[f"bias/layer{i}_mean"] = attn._last_log_fgate_mean
                stats[f"bias/layer{i}_std"] = attn._last_log_fgate_std
        return stats
